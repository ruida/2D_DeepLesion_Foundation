...
def retrieve(q,train,k,exclude_id=None):
    vals=[]
    for r in train:
        if exclude_id and r['id']==exclude_id: continue
        vals.append((r,sim(q,r)))
...
def main():
    ...
    # Precompute max-k neighbors once.
    val_neigh=[retrieve(q,train,a.max_k) for q in val]
    grid=[]
    ...
    for split,examples in data.items():
        ...
        for ex,q in zip(examples,rr):
            nn=retrieve(q,train,cfg['k'],exclude_id=q['id'] if split=='train' else None)
