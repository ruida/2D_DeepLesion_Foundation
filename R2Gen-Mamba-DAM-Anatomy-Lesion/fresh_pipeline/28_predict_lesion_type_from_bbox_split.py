...
def load_label_map(checkpoint, label_map_path):
    ...
    if label_map_path is None:
        raise RuntimeError(
            "No label map found in checkpoint. "
            "Provide --label_map."
        )
...
def normalize_label_map(label_map):
    ...
    if not isinstance(label_map, dict):
        raise RuntimeError(
            "Label map must be a dict or list."
        )
...
def main():
    ...
    if args.prediction_source:
        ...
        prediction_source = (
            "gt_bbox_crop_resnet18_in_sample"
            if args.split == "train"
            else "gt_bbox_crop_resnet18"
        )
    else:
        prediction_source = "bbox_crop_resnet18"
    ...
    with torch.no_grad():
        for example in tqdm(
            rows,
            desc=f"Predict lesion type [{args.split}]",
        ):
