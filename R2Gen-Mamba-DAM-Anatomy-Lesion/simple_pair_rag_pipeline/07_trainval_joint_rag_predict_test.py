...
def evaluate_queries(queries, reference, cfg, weights, exclude_self=False):
    exact = exact_table(reference)
    eval_rows = []
    for q in queries:
        target, _, _, _, _ = predict_one(
            q,
            reference,
            exact,
            cfg,
            weights,
            exclude_id=q["id"] if exclude_self else None,
        )
        eval_rows.append(
            {
                "pred_a": target[0],
...
def main():
            ...
            for exact_purity in (0.55, 0.65):
                for joint_purity in (0.45, 0.55, 0.65):
                    for min_similarity in (0.30, 0.40, 0.50):
                        ...
                        m = evaluate_queries(val, train, cfg, weights)
                        grid.append({"config": cfg, "metrics": m, "objective": objective(m)})
    ...
    for original, q in zip(data["test"], test):
